...
const childDetails = ref({title: "test", description: "test"});
...
const handleClick = (item: any) => {
    ...
    childDetails.value.title = "changed title";
    childDetails.value.description = "changed description";
}
...
<template>
    <div>
        ...
        <div class="w-full max-h-600px overflow-y-scroll overflow-x-hidden">
            <div ref="cardsRef" v-motion-slide-right
                ...
                <div v-for="(items, index) in displayedCards" :key="index"
                    ...
                    @click="handleClick(items)" @mouseenter="assetsDisplay(index)" @mouseleave="assetsDisplay('')" >
        ...
        <Dialog :open="cardItem" @close="setCardOpen" class="z-50">
                        <CardItem :title="childDetails.title" :description="childDetails.description" />
                        <button @click="setCardOpen">Cancel</button>
                    </Dialog>
    </div>
</template>
